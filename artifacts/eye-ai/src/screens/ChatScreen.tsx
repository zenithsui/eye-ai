...
import { ArrowLeft, Mic, Send, History, SquarePen, StopCircle, ChevronDown, Copy, Share2, RefreshCw, Zap, Paperclip } from 'lucide-react';
...
export function ChatScreen() {
  ...
  const shareMsg = (content: string) => {
    if (navigator.share) {
      navigator.share({ title: 'Eye AI', text: content }).catch(() => {});
    } else {
      copyMsg(content);
    }
    setContextMenu(null);
    ...
    <div className="screen screen-enter" style={{ background: BG }} onClick={() => { setShowMenu(false); setContextMenu(null); }}>
              ...
              <div className="msg-ai-wrapper" onContextMenu={e => showCtx(e, m.id, m.content, m.role)}>
                ...
                <div className="ai-msg-col">
                  ...
                  <div className="msg-action-row ai-actions">
                    ...
                    <button className="msg-action-btn" onClick={() => shareMsg(m.content)} title="Share"><Share2 size={11} /><span>Share</span></button>
      ...
      {contextMenu && (
        ...
        >
          ...
          <button onClick={() => shareMsg(contextMenu.content)}><Share2 size={13} /> Share</button>
      ...
      <div className="chat-input-bar border-t border-white/[0.06]"
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            type="button"
            onClick={() => showToast('📎 File attach jald aayega!', 'switch')}
            className="attach-btn flex-shrink-0"
            title="Attach file"
          >
            <Paperclip size={18} />
          </button>
